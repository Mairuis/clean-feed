...
Clean Feed 是一个跨视频网站的信息流净化浏览器扩展。v1 支持 YouTube 和 Bilibili，核心功能是：

- 正则表达式过滤：AI 根据用户的一段描述生成「规则解释 + 正则表达式」，插件把视频信息整理成带 `§` 标记的文本后执行正则匹配。
- 平台自动反馈：对已经被过滤的卡片，自动尝试点击“不感兴趣”或“点踩”。
- 屏蔽短视频：以开关形式屏蔽 YouTube Shorts 等短视频入口和卡片。
...
用户不需要手动编辑规则，只需要描述想屏蔽的内容，然后查看 AI 生成的正则配置。
